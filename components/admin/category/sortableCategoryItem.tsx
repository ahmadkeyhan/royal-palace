...
export default function SortableCategoryItem({ category, onEdit, onDelete, sortDisabled }: SortableCategoryItemProps) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="mb-3 touch-manipulation font-ravi">
      <Card className={`overflow-hidden py-0 bg-${category.menu === "restaurant" ? "regal_green" : "white"} ${isDragging ? "shadow-lg" : ""}`}>
        <CardContent className="p-0">
          <div className={`p-3 flex flex-row-reverse justify-between items-center bg-${category.menu === "restaurant" ? "regal_green" : "white"} text-${category.menu === "restaurant" ? "white" : "text_royal_green"}`}>
            ...
            <div className="flex flex-row-reverse gap-2">
              ...
              <Button variant="ghost" size="sm" className={`group bg-${category.menu === "restaurant" ? "regal_green" : "white"} hover:bg-red-500`} onClick={() => onDelete(category.id, category.name)}>
